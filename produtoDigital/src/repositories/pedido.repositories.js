...
function createPedidoRepository(id_cliente, produtoId, datapedido, quantidade_pedido) {
  return new Promise((resolve, reject) => {
    db.run(
      `INSERT INTO pedido (id_cliente, id_produto, data_pedido, quantidade_pedido) 
       VALUES (?, ?, ?, ?)`,
      [id_cliente, produtoId, datapedido, quantidade_pedido],
      function (err) {
        if (err) {
          reject(err);
        } else {
...
function findAllPedidosRepository() {
  return new Promise((resolve, reject) => {
    db.all(
      `SELECT pedido.id_pedido, pedido.data_pedido, pedido.id_cliente, pedido.id_produto, pedido.quantidade_pedido
       FROM pedido 
      `,
      [],
      (err, rows) => {
...
function findPedidoByIdRepository(pedidoId) {
  return new Promise((resolve, reject) => {
    db.get(`SELECT id_cliente, id_produto, data_pedido, quantidade_pedido
            FROM pedido WHERE 
            id_pedido = ?`, [pedidoId], 
    (err, row) => {
      if (err) {
